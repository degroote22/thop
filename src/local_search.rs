...
pub fn two_opt(
    ...
    loop {
        ...
        for (index, old_city) in old_route.iter().enumerate() {
            ...
            for city in switchable_cities.iter() {
                ...
                if result.okay {
                    ...
                    let maybe_items = instance.get_items_in_city(city).map(|items| {
                        let mut new_vec = items.clone();
                        new_vec.sort_by(|a, b| (a.weight / a.profit).cmp(&(b.weight / b.profit)));
                        new_vec
                    });
        ...
        // se o algoritmo rodou e não houve diferença na rota
        // retorna-se a rota, senao ainda pode haver chance de melhora
        if utils::vec_compare(&route, &old_route) {
            return (route.clone(), hash);
        }
    }
}
...
mod test_two_opt {
    ...
    fn integration() {
        ...
        let (final_route, asked_items_hash) = two_opt(&instance, route, hash, false);
